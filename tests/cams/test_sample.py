...
def test_sample_creation(run_date: dt.date, lead_time: int):
    sample = Sample(run_date, models=["chimere","mocage"], lead_times=[lead_time], species=["O3"], levels=[0])
...
def test_sample_str():
    sample = Sample(dt.date(2022, 7, 22), models=["chimere","mocage"], lead_times=[15], species=["O3"], levels=[0])
    result = str(sample)
...
def test_sample_paths():
    ...
    assert sample.target_paths == [Path("reanalysis/cams.eaq.ira.ENSa.o3.l0.2022-07.nc")]
...
def test_sample_is_valid_true(tmp_dataset_dir: Path):
    ...
    input_path = tmp_dataset_dir / "mocage/2022_07_22-CO_NO2_PM10_PM25_SO2_O3-0m-0-96h.netcdf"
    ...
    sample = Sample(dt.datetime(2022, 7, 22), models=["mocage"], lead_times=[15], species=["O3"], levels=[0], processed_dir=tmp_dataset_dir)
