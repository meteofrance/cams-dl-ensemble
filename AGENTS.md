...
This is the `cams-dl-ensemble` project, dedicated to post-processing the forecast of an ensemble of 11 atmospheric composition models, using Deep Learning techniques.
...
- load the `python-code-formating` skill and
  run `uvx ruff@0.15.9 format` then `uvx ruff@0.15.9 check --fix` on the changed files.
- Load the `python-type-checking` skill and run `uv run pyright` on the changed files.
- Fix any errors the checks raise, then re-run both until no errors are raised.
