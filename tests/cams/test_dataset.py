...
def test_cams_dataset_no_valid_samples(setup_cams_directories: Path):
    ...
    #input_ print((setup_cams_directories ).glob("netcdf"))
...
def test_cams_dataset_filtering_by_date(setup_cams_directories: Path):
    ...
    for date in dates:
        ...
        target_path = setup_cams_directories / f"target/{date.strftime('%Y_%m_%d_%H')}.netcdf"
        create_dummy_input_netcdf(input_path)
...
def test_cams_dataset_getitem(setup_cams_directories: Path):
...
def test_cams_dataset_single_sample(setup_cams_directories: Path):
    """Test dataset with a single valid sample."""
    # Create single valid sample
    input_path = setup_cams_directories / "input/2022_01_01.netcdf"
    target_path = setup_cams_directories / "target/2022_01_01_15.netcdf"
