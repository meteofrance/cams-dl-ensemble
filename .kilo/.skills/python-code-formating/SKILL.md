...
description: Python code formatting with uvx ruff@0.15.9. Fast, Black-compatible formatter. Use after any Python modification to format and lint.
---

# uvx ruff formatting

Format and lint Python code with `uvx ruff@0.15.9`. Use after any Python file
...
The version is pinned to `0.15.9` so formatting stays reproducible.
...
uvx ruff@0.15.9 check --fix
```

After a Python file modification, you may run these commands on the changed files only:

```bash
uvx ruff@0.15.9 format <changed file(s)>
uvx ruff@0.15.9 check --fix <changed file(s)>
```
