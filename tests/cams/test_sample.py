...
def test_sample_creation(run_date: dt.datetime, lead_time: int):
    sample = Sample(run_date, lead_time, specie="O3", level=0)
...
def test_sample_str():
    sample = Sample(dt.datetime(2022, 7, 22, 12), lead_time=15, specie="O3", level=0)
    result = str(sample)
...
def test_sample_paths():
    sample = Sample(dt.datetime(2022, 7, 22), lead_time=15, specie="O3", level=0)
...
def test_sample_is_valid_true(tmp_dataset_dir: Path):
    ...
    sample = Sample(dt.datetime(2022, 7, 22), lead_time=15, specie="O3", level=0, processed_dir=tmp_dataset_dir)
