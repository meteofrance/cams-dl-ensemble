...
def test_MeanSquaredError(preds: NamedTensor, target: NamedTensor) -> None:
    ...
    torch.testing.assert_close(
        cams_metric(preds, target), tm_metric(preds.tensor, target.tensor)
    )
...
def test_metrics_wrapper(
    metric_cls: tm.Metric, expected: float, preds: NamedTensor, target: NamedTensor
) -> None:
    """Testing all classification metrics wrapper."""
    metric = metric_cls(feature="O3", threshold=120)
    torch.testing.assert_close(
        metric(preds, target), torch.tensor(expected), rtol=1e-3, atol=1e-5
    )
...
def test_metrics_FalseAlarmRate(preds: NamedTensor, target: NamedTensor) -> None:
    ...
    torch.testing.assert_close(
        metric(preds, target), torch.tensor(0.5), rtol=1e-3, atol=1e-5
    )
